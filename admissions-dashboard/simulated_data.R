...
school_fct <- as.factor(c('UT Austin', 'Texas A&M', 'Texas Tech', 'Texas State', 'UT Brownsville', 'UT Tyler', 'Houston', 'Rice', 'UT Elpaso'))
...
is_accepted <- function(hs_grad_type, gpa, act, num_adv) {
...
hs_grad_type_fct <- sample(hs_grad_type_fct, size=size, replace=T)
gpa <- runif(n=size, min=2.0, max=4.0)
act <- sample(10:36, size=size, replace=T)
num_adv <- sample(0:10, size=size, replace=T)
num_dual_credit <- sample(0:10, size=size, replace=T)
accepted <- sapply(1:size, function(i){
  is_accepted(hs_grad_type_fct[i], gpa[i], act[i], num_adv[i])
})
reg_data <- data.frame(id=1:size, 
                      accepted=accepted, 
                      school_fct=sample(school_fct, size=size, replace=T),
                      hs_grad_type_fct=hs_grad_type_fct,
                      num_adv=num_adv,
